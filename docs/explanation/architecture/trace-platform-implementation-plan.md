...
以下 phase-4 workstreams 已完成並進入 `main`：

- `Simulation` 不再依賴 live session result state
- `Post Processing` 不再依賴 page-local latest raw result
- `Characterization` 不再以 page-local run state 作為 authority
- UI / API / CLI 都只建立 persisted execution boundary
- backend workers 只讀 persisted metadata DB + TraceStore
- local multi-user auth 與 public `/api/v1/*` 已對齊到 persisted task/result model
- dual worker lanes 已固定為 `sc-worker-simulation` 與 `sc-worker-characterization`
- startup reconcile / restart recovery 已有 safe baseline

## Current Runtime Contract

WS10 後的本地 runtime 拓樸固定為：
...
### Workstream A: Persisted Orchestration for Trace-Producing Flows (Completed)
...
- `/simulation` submit / polling / refresh recovery 全部以 persisted task / batch authority 為準
- `Simulation Results` / `Post Processing Results` 都從 persisted batch + TraceStore 重建
- `Run Post Processing` 已可從 saved raw batch rerun，不再依賴 page-local `latest_*` state
- live session state 只保留 transient UX draft / preview，不再是 workflow authority

### Workstream B: Persisted Orchestration for Analysis Flows (Completed)
...
### Workstream C: Platform Acceptance Matrix (Completed)
...
| persisted simulation run | implemented | save-only / cache-hit / rerun / reconnect 後仍能從 persisted batch 繼續 | 以 CLI + worker + page recovery regression 維持 |
| persisted post-processing run | implemented | saved raw batch 可直接重新 post-process；不依賴 live session | 以 CLI + page recovery regression 維持 |
| persisted characterization run | implemented | refresh / reconnect 後仍能從 `AnalysisRunRecord` + task state 回復 | 擴張更多 heavy-work analysis families |
...
4. startup / docs / regression hardening
