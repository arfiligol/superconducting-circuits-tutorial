...
1. **為什麼看 Im(Y)?**
   ...
   > 深入閱讀：[[../explanation/physics/resonance-extraction.md#1-admittance-method|Resonance Extraction: Admittance Method]]

2. **為什麼擬合模型?**
   提取出的 $f_0$ 會隨 $L_{jun}$ (Junction Inductance) 變化。透過擬合 $f_0(L_{jun})$ 曲線，我們可以反推電路參數。
   $$ f = \frac{1}{2\pi\sqrt{(L_{jun}/2 + L_s)C_{eff}}} $$
   > 深入閱讀：[[../explanation/physics/lc-resonance-model.md|LC Resonance Model]]
