...
文件設計規範索引，確保文件的一致性與可維護性（對齊本專案 MkDocs Material + i18n 設定）。
...
| [Maintenance](maintenance.md) | 雙語同步 + Frontmatter 更新 + MkDocs 檢查 | [#agent-rule](maintenance.md#agent-rule) |
...
- **Maintenance**: 雙語同步 + Frontmatter 更新 + MkDocs 檢查（見 `maintenance.md`）
- 詳細規範請以各子文件為準。
```
