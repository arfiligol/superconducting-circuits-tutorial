...
本專案採用 **Native Separate Builds**：

- 設定檔：`zensical.toml`（zh-TW）與 `zensical.en.toml`（en）
- 主編輯來源：`docs/`（同一路徑下以 `.md` + `.en.md` 成對維護）
- staging 來源：`docs/docs_zhtw/`、`docs/docs_en/`（由 `./scripts/prepare_docs_locales.sh` 自動重建）
...
!!! warning "同步規則"
    - 日常編輯一律在 `docs/` 進行（`.md` 與 `.en.md` 必須同步）
    - 禁止直接修改 `docs/docs_zhtw/` 與 `docs/docs_en/`（會在 prepare 階段被覆蓋）
    - 刪除/移動頁面時，必須同步更新雙語對應檔與兩份 TOML 導覽
...
3. 所有文件修改都在 `docs/`，並維持 `.md` / `.en.md` 成對一致。
4. 所有 CI / README / Contributing 文件必須與上述流程一致，避免把 staging tree 誤認為主編輯來源。
...
- 主來源樹為 `docs/`（雙語成對檔）
- 由 prepare 腳本生成兩套 locale staging：`docs/docs_zhtw/`、`docs/docs_en/`
...
!!! tip "常見問題"
    ...
    - 若 build 失敗並提示找不到頁面，先檢查兩份 TOML `nav` 是否仍指向存在於 `docs/` 的來源檔。
...
- **Verify**: run `./scripts/prepare_docs_locales.sh` before both locale builds; then `uv run --group dev zensical build -f zensical.toml` and `uv run --group dev zensical build -f zensical.en.toml`
```
