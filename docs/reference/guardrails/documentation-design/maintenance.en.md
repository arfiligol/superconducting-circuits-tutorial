...
- build staging trees: `docs_zh/` and `docs_en/` (generated by `./scripts/prepare_docs_locales.sh`)
- site output: `site/` (zh-TW) and `site/en/` (en)
...
!!! warning "Sync rules"
    ...
    - Never edit `docs_zh/` or `docs_en/` directly; they are generated staging trees
...
- build-time generation of `docs_zh/` / `docs_en/`
- two builds that emit `site/` and `site/en/`
...
!!! tip "Common issues"
    ...
    - If build fails because `docs_zh/` or `docs_en/` is missing, rerun `./scripts/prepare_docs_locales.sh`.
