...
def get_session() -> SessionResponse:
    return _build_session_response(get_session_service().get_session())
...
def list_datasets(
    *,
    ...
    return [
        ...
        for summary in get_dataset_service().list_datasets(
            DatasetListQuery(
                family=family,
                status=status,
                sort_by=sort_by,
...
def list_tasks(
    *,
    ...
    return [
        ...
        for task in get_task_service().list_tasks(
            TaskListQuery(
                status=status,
                lane=lane,
                scope=scope,
...
def get_task(task_id: int) -> TaskDetailResponse:
    return _build_task_detail_response(get_task_service().get_task(task_id))
...
def get_circuit_definition(definition_id: int) -> CircuitDefinitionDetailResponse:
    return _build_circuit_definition_detail_response(
        get_circuit_definition_service().get_circuit_definition(definition_id)
    )
