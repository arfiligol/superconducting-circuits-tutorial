...
def test_designation_repository_unique_lookup_and_exclusion() -> None:
    with _memory_session() as session:
        ...
        found = uow.designations.find_unique(
            dataset_id=design.id,
            designated_name="f_q",
            source_analysis_type="fit",
            source_parameter_name="mode_1_ghz",
        ...
        excluded = uow.designations.find_unique(
            dataset_id=design.id,
            designated_name="f_q",
            source_analysis_type="fit",
            source_parameter_name="mode_2_ghz",
...
def test_derived_parameter_repository_exact_and_prefix_lookups() -> None:
    with _memory_session() as session:
        ...
        by_name = uow.derived_params.get_by_dataset_and_name(design.id, "mode_1_ghz")
        assert by_name is not None
        assert by_name.id == mode_base.id

        by_exact = uow.derived_params.get_by_dataset_method_and_name(
            design.id,
            "fit",
            "mode_1_ghz_b0",
        )
        ...
        by_prefix = uow.derived_params.get_first_by_dataset_method_name_prefix(
            design.id,
            "fit",
            "mode_2_ghz",
        )
