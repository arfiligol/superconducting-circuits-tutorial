...
def test_trace_record_design_id_prefers_canonical_design_scope() -> None:
    ...
    assert trace_record_dataset_id(record) == 42
    assert normalized.design_id == 42
    assert normalized.dataset_id == 42


def test_trace_record_design_id_uses_legacy_dataset_scope_when_needed() -> None:
    record = {
        "id": 8,
        "dataset_id": 17,
    ...
    assert trace_record_design_id(record) == 17
    assert trace_record_dataset_id(record) == 17
    assert normalized.design_id == 17
