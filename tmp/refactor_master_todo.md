...
- [x] 2.1 建立 simulation/characterization use-case 層（DTO in/out）
  - Page 只保留 I/O、state mapping、render call
- 已完成：`app/services/characterization_runner.py` 與 `app/services/post_processing_runner.py` 分別承接 Characterization analysis run 與 Simulation post-processing pipeline，page 僅保留 UI I/O 與結果映射。
- [x] 2.2 移除 page 直接組 repository 查詢參數的路徑
  - 改由 use-case / query object 層輸入輸出
  - 已完成：Characterization trace scope query 全數透過 `TraceIndexPageQuery`（`app/services/characterization_trace_scope.py`）封裝，page 不再拼裝 repository 分頁/篩選 kwargs。
- [x] 2.3 建立 pipeline step registry（post-processing / analysis 共用策略）
  - 讓新研究步驟可插拔，不走 page if/else 擴張
- 已完成：新增 `app/services/post_processing_step_registry.py`，集中 `step options/default/serialize/normalize/preview/execute`，`simulation` page 改為 registry-dispatch 執行 CT/Kron。
