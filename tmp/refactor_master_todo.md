...
- [ ] 4.3 建立 typed service interfaces
  ...
  - 進度：`analysis_registry` 已導入 `AnalysisDescriptor`/`AnalysisConfigField` typed interface 與 `list_dataset_analyses()` API；`dataset_profile` 已導入 `DatasetProfile` typed payload。
